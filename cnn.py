...
class CNN():
    ...
    def __init__(self, num_features, num_historical_days, is_train=True):
        ...
        with tf.compat.v1.variable_scope("cnn"):
            ...
            if is_train:
                relu = tf.nn.dropout(relu, rate = 1 - (self.keep_prob))
            print(relu)


            k2 = tf.Variable(tf.random.truncated_normal([3, 1, 16, 32],
                stddev=0.1,seed=SEED, dtype=tf.float32))
            b2 = tf.Variable(tf.zeros([32], dtype=tf.float32))
            conv = tf.nn.conv2d(input=relu, filters=k2,strides=[1, 1, 1, 1],padding='SAME')
            ...
            k3 = tf.Variable(tf.random.truncated_normal([3, 1, 32, 64],
                stddev=0.1,seed=SEED, dtype=tf.float32))
            b3 = tf.Variable(tf.zeros([64], dtype=tf.float32))
            conv = tf.nn.conv2d(input=relu, filters=k3, strides=[1, 1, 1, 1], padding='VALID')
            ...
            W1 = tf.Variable(tf.random.truncated_normal([18*1*64, 32]))
            b4 = tf.Variable(tf.random.truncated_normal([32]))
            h1 = tf.nn.relu(tf.matmul(flattened_convolution, W1) + b4)


            W2 = tf.Variable(tf.random.truncated_normal([32, 2]))
            logits = tf.matmul(h1, W2)

            #self.accuracy = tf.metrics.accuracy(tf.argmax(self.Y, 1), tf.argmax(logits, 1))
            self.accuracy = tf.reduce_mean(input_tensor=tf.cast(tf.equal(tf.argmax(input=self.Y, axis=1), tf.argmax(input=logits, axis=1)), tf.float32))
            self.confusion_matrix = tf.math.confusion_matrix(labels=tf.argmax(input=self.Y, axis=1), predictions=tf.argmax(input=logits, axis=1))
            tf.compat.v1.summary.scalar('accuracy', self.accuracy)
            theta_D = [k1, b1, k2, b2, k3, b3, W1, b4, W2]           
            
            # D_prob = tf.nn.sigmoid(D_logit)

        self.loss = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(self.Y), logits=logits))
        ...
        self.optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=0.0001).minimize(self.loss)
