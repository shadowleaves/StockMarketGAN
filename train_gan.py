...
class TrainGan:
    ...
    def __init__(self, num_historical_days, batch_size=128):
        ...
        for file in files:
            ...
            df = ((df - df.rolling(num_historical_days).mean().shift(-num_historical_days)) / (df.rolling(num_historical_days).max().shift(-num_historical_days) - df.rolling(num_historical_days).min().shift(-num_historical_days)))
    ...
    def train(self, print_steps=100, display_data=100, save_steps=1000):
        ...
        for i, X in enumerate(self.random_batch(self.batch_size)):
            if i % 1 == 0:
                _, D_loss_curr, D_l2_loss_curr = sess.run([self.gan.D_solver, self.gan.D_loss, self.gan.D_l2_loss], feed_dict={self.gan.X: X, self.gan.Z: self.gan.sample_Z(self.batch_size, 200)})
                D_loss += D_loss_curr
