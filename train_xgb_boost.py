...
class TrainXGBBoost:
    ...
    def __init__(self, num_historical_days, days=10, pct_change=0):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            self.test_data = np.array(self.test_data).astype(np.float32)
    ...
    def train(self, reset=False):
        ...
        clf = xgb.train(params, train, 200, evals=watchlist, early_stopping_rounds=100)
        joblib.dump(clf, 'models/clf.pkl')

        pred = list(map(lambda x: int(x[1] > .5), clf.predict(test)))
        cm = confusion_matrix(self.test_labels, pred)
        print(cm)
